  ...
  */
  ...
  /* ---------------------------
    ...
    function drawImageCover(ctxLocal, img, x, y, boxW, boxH, offsetX = 0, offsetY = 0, scale = 1, rotateLandscapeToPortrait = true) {
      ...
      let drawW, drawH;
      // AUTO FIT FULL FRAME (LAPRAK OTOMATIS)
  const scaleBase = Math.max(boxW / iw, boxH / ih);

  drawW = iw * scaleBase * scale;
  drawH = ih * scaleBase * scale;
...
if (downloadPdf) downloadPdf.onclick = async () => {
  ...
  try {
    ...
    function showLoading() {
      document.getElementById("loadingOverlay_kolase").classList.add("active");
    }
    
    function hideLoading() {
      document.getElementById("loadingOverlay_kolase").classList.remove("active");
    }
